#!/usr/bin/env python3
"""Generate PvP Dimensions' mod menu icon: two swords crossed in front of a lit frame.

A fight, and the portal it happens on the other side of. Every pixel is cut from the
vanilla textures: obsidian, the nether portal's first frame, the iron and diamond swords.
...
OUT = os.path.join(HERE, "src/main/resources/assets/pvp-dimensions-justfatlard/icon.png")
...
def build_icon():
    ...
    sheet = vanilla("block/nether_portal.png")[:16]
    icon = [[dim(px, 1.2) for px in row] for row in vanilla("block/obsidian.png")[:16]]

    # The frame's lit face, an obsidian border's width in from the edge.
    for y in range(2, 14):
        for x in range(3, 13):
            if y in (2, 13) or x in (3, 12):
                icon[y][x] = outline if (x + y) % 2 else dark
            else:
                icon[y][x] = sheet[y][x]

    # Iron from bottom left to top right, diamond mirrored across it.
    iron = vanilla("item/iron_sword.png")
    diamond = vanilla("item/diamond_sword.png")
    for y in range(16):
        for x in range(16):
            if iron[y][x][3] > 0:
                icon[y][x] = iron[y][x]
            mirrored = diamond[y][15 - x]
            if mirrored[3] > 0:
                icon[y][x] = mirrored
    return scale(icon, 8)
